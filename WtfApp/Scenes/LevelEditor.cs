    ...
    public class LevelEditor : Scene
    {
        ...
        public LevelEditor(Rectangle sceneRectangle, SaveLoadLevel sll):this(sceneRectangle)
        {
            if (sll != null)
            {
                ...
                for (int b = 0; b < sceneButtons.Count; b++)
                {
                    for (int i = 0; i < this.availableUserWorldObj.Count; i++)
                    {
                        if (sceneButtons[b].Name == Enum.GetName(typeof(WTFHelper.WORLD_OBJ_TYPE), this.availableUserWorldObj[i]))
                        {
                            sceneButtons[b].borderStyle = BorderStyle.SOLID;
                        }
                    }
                }
        ...
        public LevelEditor(Rectangle sceneRectangle) : base(WTFHelper.SCENES.MAIN_MENU, sceneRectangle)
        {
            ...
            worldMenuBtnRect = new Rectangle(Main.screenBounds.Width - (int)(Main.screenBounds.Width * menuPartWidth),
                                        Main.screenBounds.Top,
                                        (int)(Main.screenBounds.Width * menuPartWidth),
                                        (int)(Main.screenBounds.Height * 0.55));

            menuBtnRect = new Rectangle(Main.screenBounds.Width - (int)(Main.screenBounds.Width* menuPartWidth),
                                        Main.screenBounds.Height - (int)(Main.screenBounds.Height*0.45),
                                        (int)(Main.screenBounds.Width * menuPartWidth),
                                        (int)(Main.screenBounds.Height * 0.45));
            ...
            { 
                AddButton(WTFHelper.worldObjTextures.ElementAt(i).Key, "" , new Rectangle(worldMenuBtnRect.Left+ (int)Math.Ceiling(i / countWorldItemsInCol*1.0) * worldBtnItemSize,
                    worldMenuBtnRect.Top+ i%countWorldItemsInCol* worldBtnItemSize,
                    worldBtnItemSize, worldBtnItemSize),
                    WTFHelper.worldObjTextures.ElementAt(i).Value, WTFHelper.worldObjTextures.ElementAt(i).Value);
                sceneButtons.GetButton(WTFHelper.worldObjTextures.ElementAt(i).Key).ChangeDefaultColor(Color.FromNonPremultiplied(255, 255, 255, WTFHelper.alpha));
            }

            AddButton("SAVE", "SAVE", new Rectangle(menuBtnRect.X, menuBtnRect.Y, menuBtnRect.Width, buttonsHeight));
            AddButton("TEST", "TEST", new Rectangle(menuBtnRect.X, menuBtnRect.Y+ (buttonsHeight+buttonsInterval), menuBtnRect.Width, buttonsHeight));
            AddButton("BACK", "BACK", new Rectangle(menuBtnRect.X, menuBtnRect.Y+ (buttonsHeight + buttonsInterval)*2, menuBtnRect.Width, buttonsHeight));

            AddButton("W-", "W-", new Rectangle(menuBtnRect.Left, menuBtnRect.Bottom- (buttonsHeight + buttonsInterval) * 2, menuBtnRect.Width/2- buttonsInterval/2, buttonsHeight));
            AddButton("W+", "W+", new Rectangle(menuBtnRect.Right - menuBtnRect.Width/2+ buttonsInterval/2, menuBtnRect.Bottom - (buttonsHeight + buttonsInterval)*2, menuBtnRect.Width/2- buttonsInterval/2, buttonsHeight));
            AddButton("H-", "H-", new Rectangle(menuBtnRect.Left, menuBtnRect.Bottom - buttonsHeight-buttonsInterval, menuBtnRect.Width/2- buttonsInterval/2, buttonsHeight));
            AddButton("H+", "H+", new Rectangle(menuBtnRect.Right- menuBtnRect.Width/2+ buttonsInterval/2, menuBtnRect.Bottom - buttonsHeight - buttonsInterval, menuBtnRect.Width/2- buttonsInterval/2, buttonsHeight));
        }
            ...
            for (int y = 0; y < worldSizeY; y++)
            {
                for (int x = 0; x < worldSizeX; x++)
                {
                    ...
                    DrawHelper.DrawRectagle(spriteBatch, Color.FromNonPremultiplied(50, 50, 50, WTFHelper.alpha),
                        (int)offsetDraw.X + x * MainG.cellSize,
                        (int)offsetDraw.Y + y * MainG.cellSize,
                        MainG.cellSize, MainG.cellSize, 5);
                }
            }
        }
