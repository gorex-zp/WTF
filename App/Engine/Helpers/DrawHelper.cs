...
namespace WtfApp
{
    ...
    {
        public static readonly Color defColor = Color.FromNonPremultiplied(255, 255, 255, WTFHelper.alpha);
        private static Texture2D emptyTexture;
        public static Texture2D GetTexture(SpriteBatch spriteBatch=null)
        {
            if (emptyTexture == null)
            {
                emptyTexture = new Texture2D(spriteBatch.GraphicsDevice, 1, 1, false, SurfaceFormat.Color);
                emptyTexture.SetData(new[] { defColor });
            }
            return emptyTexture;
        }

        public static SpriteFont spriteFont,spriteFontBig;
        ...
        {
            //int gBorderF =  (borderWidth >= Math.Ceiling(height / 2.0) ? (int)Math.Ceiling(height / 2.0) : borderWidth);
            //int vBorderF =  (borderWidth >= Math.Ceiling(width / 2.0)  ? (int)Math.Ceiling(width / 2.0)  : borderWidth);

            int gBorder = (borderWidth >= height / 2 ? height / 2 : borderWidth);
            int vBorder = (borderWidth >= width / 2? width / 2 : borderWidth);

            spriteBatch.DrawLine(new Vector2(x - gBorder / 2, y), width + gBorder, 0, color, borderWidth);
            spriteBatch.DrawLine(new Vector2(x - gBorder / 2, y + height), width + gBorder, 0, color, borderWidth);
            spriteBatch.DrawLine(new Vector2(x, y), height, MathHelper.PiOver2, color, borderWidth);
            spriteBatch.DrawLine(new Vector2(x+width, y), height, MathHelper.PiOver2, color, borderWidth);
        ...
        public static void DrawGrid(this SpriteBatch spriteBatch, Rectangle pos, Point cellCount,Color color,int thickness)
        {
            Vector2 v = pos.Location.ToVector2();   
            for (int y = 0; y < cellCount.Y+1; y++)
            {
                spriteBatch.DrawLine(v, pos.Width,0,color,thickness);
                v.Y += pos.Height/ cellCount.Y;
            }
            v = pos.Location.ToVector2();
            for (int x = 0; x <  cellCount.X+1; x++)
            {  
                spriteBatch.DrawLine(v, pos.Height, MathHelper.PiOver2, color, thickness);
                v.X += pos.Width/cellCount.X;
            }
        }
