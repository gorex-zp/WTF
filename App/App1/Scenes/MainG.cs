...
namespace WtfApp.App1
{
    public class MainG : Scene
    {
        ...
        public bool isStopped = true; //change to TRUE
        ...
        public MainG(Rectangle sceneRectangle,SaveLoadLevel sll) : base(WTFHelper.SCENES.GAME, sceneRectangle)
        {
            ...
            for (int i = 0; i < availableWorldObjects.Length; i++)
            {
               /* if (i == countWorldItemsInCol)
                ...
                Button btn = new  Button(availableWorldObjects[i].ToString(), "",
                        new Rectangle(worldMenuBtnRect.Left + (int)Math.Ceiling(i / countWorldItemsInCol *1.0) * (int)(worldItemSize * (worldMenuBtnScale + 0.1f)),
                                      worldMenuBtnRect.Top + i % countWorldItemsInCol * (int)(worldItemSize * (worldMenuBtnScale + 0.1f)),
                                      (int)(worldItemSize* worldMenuBtnScale),
                                      (int)(worldItemSize* worldMenuBtnScale)),
            ...
            AddComponent(new Button("START_BTN", "START", new Rectangle(menuBtnRect.X, menuBtnRect.Y, menuBtnRect.Width, buttonsHeight)));
        ...
        public bool CheckRotate(int cX, int cY, int x, int y, BaseObject.ROTATE_DIRRECTION direction, bool needBlockRotateArea)
        {
            ...
            for (double i = 0; i <= MathHelper.PiOver2 && radius>0; i += MathHelper.PiOver2/radius/10 /* MathHelper.PiOver2/radius*6.0*/)
            {
                ...
                */
                ...
                DEBUGRECTS.Add(new RectangleF(Math.Floor(ltX) * cellSize+cellSize/2, Math.Floor(ltY) * cellSize + cellSize / 2, 1 / App.screenScale, 1 / App.screenScale));
                DEBUGRECTS.Add(new RectangleF(Math.Floor(rtX) * cellSize + cellSize / 2, Math.Floor(rtY) * cellSize + cellSize / 2, 1 / App.screenScale, 1 / App.screenScale));
                DEBUGRECTS.Add(new RectangleF(Math.Floor(lbX) * cellSize + cellSize / 2, Math.Floor(lbY) * cellSize + cellSize / 2, 1 / App.screenScale, 1 / App.screenScale));
                DEBUGRECTS.Add(new RectangleF(Math.Floor(rbX) * cellSize + cellSize / 2, Math.Floor(rbY) * cellSize + cellSize / 2, 1 / App.screenScale, 1 / App.screenScale));
                
                /*
                DEBUGRECTS.Add(new RectangleF((ltX) * cellSize, ((ltY) * cellSize), 1 / App.screenScale, 1 / App.screenScale));
                DEBUGRECTS.Add(new RectangleF((rtX) * cellSize, ((rtY) * cellSize), 1 / App.screenScale, 1 / App.screenScale));
                DEBUGRECTS.Add(new RectangleF((lbX ) * cellSize, ((lbY ) * cellSize), 1 / App.screenScale, 1 / App.screenScale));
                DEBUGRECTS.Add(new RectangleF((rbX) * cellSize, ((rbY) * cellSize), 1/App.screenScale, 1/App.screenScale));
                */
        ...
        public override void Touch(Point touch, ButtonState touchState, bool isPressedMove)
        {
            ...
            if (worldObjectsRect.Contains(touch))
            {
                ...
                if (selectedWorldObj != WTFHelper.WORLD_OBJ_TYPE.EMPTY && isStopped==true)
                {
            ...
            if (isDebug == true)
            {
                for (int i = 0; i < DEBUGRECTS.Count; i++)
                {
                    spriteBatch.Draw(DrawHelper.GetTexture(),
                        destinationRectangle: DEBUGRECTS[i].GetRectangle(),
                        color: Color.FromNonPremultiplied(255, 255, 255, WTFHelper.alpha)/*,
                        layerDepth: WTFHelper.DRAW_LAYER.DEBUG.F()*/);
                }
                isPaused = true;
                ...
                spriteBatch.DrawString(DrawHelper.spriteFont, debugStr, new Vector2(1100, 0), Color.FromNonPremultiplied(0, 255, 0, 255),0,Vector2.Zero,Vector2.One,SpriteEffects.None, WTFHelper.DRAW_LAYER.ABSOLUTE_FRONT.F());
            }
        }
                            ...
                            switch (btn.Name)
                            {
                                case "START_BTN":
                                    ...
                                    else
                                    {
                                        ...
                                        isStopped = true;
                                        btn.Text = "START";
                                        ((Button)guiObjects["PAUSE_BTN"]).Text = "PAUSE";
                                    }
                                    break;
                                case "PAUSE_BTN":
                                    ...
                                    else if( this.state == SceneState.Paused)
                                    {
                                        btn.Text = "PAUSE";
                                        this.state = SceneState.Running;
                                    }
