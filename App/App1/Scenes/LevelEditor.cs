...
namespace WtfApp.App1
{
    public class LevelEditor : Scene
    {
        ...
        public LevelEditor(Rectangle sceneRectangle) : base(WTFHelper.SCENES.MAIN_MENU, sceneRectangle)
        {
            ...
            {
                Button btn = new Button(WTFHelper.worldObjTextures.ElementAt(i).Key,"", new Rectangle(worldMenuBtnRect.Left + (int)Math.Ceiling(i / countWorldItemsInCol * 1.0) * worldBtnItemSize,
                    worldMenuBtnRect.Top + i % countWorldItemsInCol * worldBtnItemSize,
                    worldBtnItemSize, worldBtnItemSize), WTFHelper.worldObjTextures.ElementAt(i).Value, WTFHelper.worldObjTextures.ElementAt(i).Value);
